...
let state={case:null,result:null,initialAnswers:[],challengeAnswers:[],submissionAttempted:false};
...
function renderResult(){const r=state.result,hasChallenge=!state.challengeAnswers.length&&r.challengeRequired&&r.challengeQuestions?.length,title=state.challengeAnswers.length?'Revised assessment':'Initial assessment';set(`<h2>${title}</h2><p class="score">${esc(r.score)}/5</p><div class="feedback-card"><h3>What is correct</h3><ul>${(r.correctPoints||[]).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>No material correct point was identified.</li>'}</ul></div><div class="feedback-card"><h3>Most important missing point</h3><p>${esc(r.missingPoint)}</p></div><div class="feedback-card"><h3>Stronger answer</h3><ul>${(r.strongerAnswer||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>${hasChallenge?challengeHtml(r.challengeQuestions):''}${recordHtml(hasChallenge)}<div class="actions" id="resultActions"></div>`);bindRecordButtons(hasChallenge);document.getElementById('resultActions').append(button('Choose another case',showCases,'secondary'))}
...
function recordHtml(hasChallenge){return `<div class="feedback-card"><h3>${hasChallenge?'Want to improve your score?':'Record score'}</h3><p>${hasChallenge?'You can record your current score now or answer the optional challenge questions for a reassessment.':'Would you like to record your score for this course?'}</p><div id="recordButtons" class="actions"></div></div>`}
function bindRecordButtons(hasChallenge){const x=document.getElementById('recordButtons');if(!x)return;x.append(button(hasChallenge?'Record current score':'Record score',showSubmit));if(hasChallenge)x.append(button('Answer optional challenge questions',showChallengeForm,'secondary'));x.append(button('Do not record',showCases,'secondary'))}
function showChallengeForm(){const qs=state.result.challengeQuestions;set(`<h2>Optional challenge question${qs.length>1?'s':''}</h2><form id="challengeForm">${qs.map((q,i)=>`<div class="question"><label for="c${i}">${i+1}. ${esc(q)}</label><textarea id="c${i}" placeholder="Enter short bullet points"></textarea></div>`).join('')}<div id="challengeMessage"></div><div class="actions"><button type="submit">Submit for reassessment</button></div></form>`);document.getElementById('challengeForm').onsubmit=async e=>{e.preventDefault();const vals=qs.map((_,i)=>document.getElementById(`c${i}`).value.trim());if(vals.some(v=>!v)){document.getElementById('challengeMessage').innerHTML='<p class="notice error">Please answer all challenge questions.</p>';return}const previousAssessment=state.result;state.challengeAnswers=vals;await callAssessment({caseId:state.case.caseId,answers:state.initialAnswers,challengeAnswers:vals,previousAssessment})}}
