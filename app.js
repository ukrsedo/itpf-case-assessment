...
function renderResult(){const r=state.result;set(`<h2>Assessment result</h2><p class="score">${esc(r.score)}/5</p><div class="feedback-card"><h3>Assessment by question</h3>${(r.questionFeedback||[]).map((x,i)=>`<div class="feedback-card"><h3>${i+1}. ${esc(x.question)}</h3><p>${esc(x.assessment)}</p></div>`).join('')}</div><div class="feedback-card"><h3>What is correct</h3><ul>${(r.correctPoints||[]).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>No material correct point was identified.</li>'}</ul></div><div class="feedback-card"><h3>Most important missing point</h3><p>${esc(r.missingPoint)}</p></div><div class="feedback-card"><h3>Stronger answer</h3><ul>${(r.strongerAnswer||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>${recordHtml()}<div class="actions" id="resultActions"></div>`);bindRecordButtons();document.getElementById('resultActions').append(button('Choose another case',showCases,'secondary'))}
function recordHtml(){return `<div class="feedback-card"><h3>Record score</h3><p>Would you like to record this score for the course?</p><div id="recordButtons" class="actions"></div></div>`}
function bindRecordButtons(){const x=document.getElementById('recordButtons');if(!x)return;x.append(button('Record score',showSubmit));x.append(button('Do not record',showCases,'secondary'))}
